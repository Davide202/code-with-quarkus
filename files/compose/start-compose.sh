...
# ==========================================
# Trova la cartella assoluta in cui si trova questo script (cioè la cartella 'compose')
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" &> /dev/null && pwd)"

# Vai alla root del progetto (un livello sopra la cartella dello script)
PROJECT_ROOT="$SCRIPT_DIR/.."
cd "$PROJECT_ROOT" || exit 1

echo "🚀 Avvio compilazione nativa Quarkus in corso nella root: $PWD..."
...
if [ $? -eq 0 ]; then
    ...
    # Torna nella cartella 'compose' in modo sicuro per lanciare Docker
